...
def test_add_success(service, mock_db_session):
    ...
    result = service.add(mock_db_session, user_id="user_123", payment_method_name="Credit Card")
...
def test_add_duplicate_raises_error(service, mock_db_session):
    ...
    with pytest.raises(ValueError, match="already exists"):
        service.add(mock_db_session, user_id="user_123", payment_method_name="Credit Card")
...
def test_delete_success(service, mock_db_session):
    ...
    mock_db_session.query.return_value.filter.return_value.first.return_value = PaymentMethod(id="pm_1")
